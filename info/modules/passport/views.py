...
def register():
    ...
    print("mobile , sms_code password is %s %s %s" %(mobile, sms_code,password))
    ...
    # 2. 从redis中获取指定手机号对应的短信验证码的
    try:
        real_sms_code = redis_store.get("SMS_" + mobile).decode(encoding='utf-8',errors='strict')
        print("real_sms_code is == %s"% real_sms_code )
    except Exception as e:
        current_app.logger.error(e)
        # 获取本地验证码失败
    ...
    print("sms_code, real_sms_code is %s , %s"%(sms_code,real_sms_code))
    # 3. 校验验证码
